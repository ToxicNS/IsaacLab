    ...
    if not is_episode_annotated_successfully:
        ...
        for signal_name, signal_flags in subtask_term_signal_dict.items():
            if not torch.any(signal_flags):
                ...
                # Find the start and end indices of the subtask
                start_index = torch.nonzero(signal_flags, as_tuple=True)[0][0].item()
                end_index = torch.nonzero(signal_flags, as_tuple=True)[0][-1].item()
                print(f'\tSubtask "{signal_name}" starts at action index: {start_index}')
                print(f'\tSubtask "{signal_name}" ends at action index: {end_index}')
